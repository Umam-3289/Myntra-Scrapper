...
class ScrapReviews:
    ...
    def extract_reviews(self,product_link):
        try:
            product_link="https://www.myntra.com/"+product_link
            ...
            for i in overallRating:
                self.product_rating_value=i.find_all("div").text
            price=prodRes_html.find_all("span",{"class":"pdp-price"})
            for i in price:
                self.product_price=i.text
            product_reviews=prodRes_html.find_all("a",{"class":"detailed-reviews-allReviews"})
    ...
    def scroll_to_load_reviews(self):
            ...
            while True:
                ...
                # Break the loop if no new content isloaded after srolling
    ...
    def extract_products(self,product_reviews: list):
        try:
            ...
            review=review_html("div",{"class":"detiled-reviews-userReviewsContainer"})
            for i in review:
                user_rating=i.find_all("div",{"class":"user-review-main user-review-showRating"})
                user_comment=i.find_all("div",{"class":"user-review-reviewTextWrapper"})
                user_name=i.find_all("div",{"class":"user-review-left"})

            reviews=[]
            for i in range(len(user_rating)):
                try:
                    rating=(user_rating[i].find("span",class_="user-review_starRating").get_text().strip())
                except:
                ...
                try:
                    name=user_name[i].find_all("span")[1].text
                except:
                    name="No name Given"
                try:
